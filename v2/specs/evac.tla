...
(*                                                                           *)
(* γ-style durability (post-2026-04-25): evacuation does NOT need a       *)
(* separate γ extension analogous to scrub.tla's Persist/Crash/Mount.    *)
(* The reason: every variable in this spec maps directly to durable       *)
(* on-disk state in the C impl —                                           *)
(*   - `device_state[d]` ↔ ub_roster[d].state (per-slot byte in UB).     *)
(*   - `replicas[b]`     ↔ each surviving alloc tree's contents (the     *)
(*                          tree is encrypted + Merkle-rooted in UB).    *)
(* Both are written atomically via sync_commit (see quorum.tla). A crash *)
(* between two evac steps leaves either the pre-step or post-step state *)
(* on disk; mount-time recovery just calls stm_sync_evacuation_step on   *)
(* whichever device is still EVACUATING. No additional durable cursor    *)
(* is needed; the alloc tree IS the cursor (its remaining entries are   *)
(* the un-evacuated blocks). Phase 5 exit criterion 4 ("rebalance       *)
(* progress persists across restart") is therefore met for the          *)
(* mirror-evacuation path by the existing format alone. The format-     *)
(* break that adds scrub-γ does NOT add new fields for evacuation.      *)
(***************************************************************************)
