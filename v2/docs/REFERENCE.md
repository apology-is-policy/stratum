...
- **Tip**: 9.7-impl-6f — clone integration tests + two bug fixes
  ...
  bootstrap-paddr / data-paddr / cold-record sets + OLD-live's cold
  records ONCE; computes snap_unique (same algorithm as 4c-iii);
  per newer snap calls `stm_snapshot_delete` → filters boot/data
  buffers vs s.view via set-difference + aggregates cold hashes;
  post-loop counted subtraction `agg_cold(H) − snap_unique(H)`
  clamped at 0. R160 P1-1 PRE-DRAIN refusal: every newer snap is
  `stm_snapshot_lookup`'d BEFORE the destructive drain — if any
  `hold_count > 0`, refuse `STM_EBUSY` as a true no-op.
  LOAD-BEARING — counted subtraction safety: every `snap_unique`
  record's drop fired the snap-aware deref routing while SOME newer
  snap was most-recent at COW time, so at least one matching entry
  lives in the union of newer snaps' cold dead-lists → `snap_unique(H)
  ≤ agg_cold(H)` per hash always; clamp is defense-in-depth.
  DISJOINT-SET property: `(OLD-live ∖ s.view) ∩
  (newer_snap.dead_list) = ∅` (a paddr in OLD-live is still-allocated-
  not-yet-dropped, so cannot appear in any newer snap's dead-list) —
  the 4b/4c live-divergence reclaims and the 4d boot/data cascade
  reclaims operate on DISJOINT paddr sets, no double-free risk.
  Best-effort per snap: a failure on one newer snap leaves the others
  reclaimed; rollback itself still succeeds. No STM_UB_VERSION bump.
  - **9.7-impl-4c-iii** (`55b3b16` + R164 close `2562529`, verdict
    0 P0/P1/P2/P3 — GREEN, no inline fixes needed):
    `fs_rollback_reclaim_cleared_dead_list_garbage` reads S's three
    dead-lists via three new non-destructive getters
    (`stm_snapshot_{dead_list, bootstrap_dead_list, cold_dead_list}_get`)
    and reclaims `snap_dead[s] \ s_view` — the case-(b) intermediate-
    COW garbage that impl-2's snap-aware COW free dumped onto S's
    dead-list under "most-recent snap" but which S's tree never
    references. Boot + data: set-difference on paddrs vs s.view's
    frozen-tree walks. Cold: per-key structural merge of OLD vs SNAP
    cold records to count `snap_unique[hash]`, then per-hash
    `deref_count = dead_S(H) − snap_unique(H)` clamped at 0 (counted
    subtraction safe HERE — every snap_unique record has a matching
    dead-list entry from its post-snap drop event, unlike 4c-ii's
    live-divergence case). Flow REORDERS: `clear_dead_lists` moves
    from pre-reclaim to post-reclaim (4b/4c/4c-ii are pure readers of
    `sidx`, observation-preserving). With this tier the rollback
    realises `to_free` minus the `newer_dead \ s_view` term. No
    STM_UB_VERSION bump.
...
- **Pre-tip-0**: Phase 9.7-impl-1c-vi (`3afa915`) — the pool-global-engine
  ...
  (`extent_root_paddr/gen/csum`, `inode_root_*`, `dirent_root_*`,
  `xattr_root_*`) are deleted; `build_uberblock` dropped its 12
  corresponding parameters + the 4 conditional `if (X_paddr != 0)`
  UB-field write blocks + the 4 `out->ub_*_root_gen = stm_store_le64(...)`
  stamps. The UB fields `ub_extent_root` / `ub_inode_root` /
  `ub_dirent_root` / `ub_xattr_root` (plus their `_gen` / `_csum`
  siblings) are now reserved-zero on every v30 write — the bytes are
  zero via `build_uberblock`'s initial `memset(out, 0, sizeof *out)`,
  uniform for every write. Mount paths ignore the four UB fields
  entirely; on-disk values from pre-1c-vi v30 writes (where the bytes
  were conditionally zero too) are read but not consulted.
  STM_UB_VERSION stays at 30. `compute_merkle_root` signature
  unchanged — four legacy csum slots fed from a shared `zero_csum`
  local. Q2 wedge preserved; ctest 64/64 GREEN standalone.

- **Pre-tip-1**: Phase 9.7-impl-1c-v — fourth module cutover (extent).
...
- **Pre-tip-2**: Phase 9.7-impl-1c-iv — third module cutover lands.
  ...
  `stm_xattr_index_attach_dataset_index` replaces the retired
  persistence API. M-engine cascade extends to cover xattr records;
  `xattr_csum` zeroed in `compute_merkle_root`; UB xattr fields
  stamped zero.

- **Pre-tip-3**: Phase 9.7-impl-1c-iii — second module cutover
...
- **Pre-tip-4**: Phase 9.7-impl-1c-ii — first module cutover
...
- **Pre-tip-5**: Phase 9.7-impl-1b — `stm_dataset_entry` gains the
...
- **Pre-tip-6**: Phase 9.7-impl-1a — `v2/src/metakey/` lib lands.
...
- **Pre-tip-5**: Phase 9.6-impl-4d — the final cutover that lands
...
- **Pre-tip-6**: 9.6-impl-4c — cut dirent + xattr to `btree_engine`
...
- **Pre-tip-7**: 9.6-impl-4b — production vtable + sync wiring +
...
- **Pre-tip-8**: 9.6-impl-4a — engine completion (delete +
...
- **Pre-tip-10**: R134 audit close (`2a117e9`). 0 P0, 0 P1, 3 P2 — all
  ...
  observing the test hang past the 30s deadline) + 6 pin_many unit
...
- **Pre-tip-11**: P9.5-PARALLEL-3 impl-3 (`7970bcf`). Ports `stm_fs_rename`
  ...
  `compound_ops_per_inode.tla::NoCircularWait` becomes load-bearing
  — under 4 inodes, opposite-direction renames (A→B vs B→A) without
  ascending lock-order would form a 4-cycle deadlock; pin_many's
  internal sort prevents it.
  - **`stm_inode_pin_many`** (NEW, generalises pin_two): caller
    supplies a `(dataset_id, ino)` array of length ≤ 16; helper sorts
    internally, refuses duplicates, pins in ascending order, maps
    handles back to caller slot order. Same lock-cycle-impossible
    guarantee as pin_two scaled to arbitrary N.
  - **`stm_fs_rename` ported**: SH lock + retry-restart loop with
    TOCTOU re-verify on BOTH src and dst dirents (generalises the
    impl-2 DELETE-shape single-dirent re-verify pattern). Re-checks
    src + dst inos AND parent dir kinds under pin; mismatch ⇒ unpin
    all + retry (FS_UNLINK_MAX_ATTEMPTS bound). Covers all five
    rename branches: regular without overwrite, regular with
    overwrite (4-inode cascade-free), NOREPLACE refusal, EXCHANGE
    (4-inode atomic dirent_swap_two), WHITEOUT.
  - **R128/R130 invariants preserved** verbatim on the overwrite
    branch: `fs_pre_inode_free_cleanup_locked` runs under dst_ino's
    pin (acquired via pin_many); post-reclaim double-commit gated on
    `dst_cleanup_did_truncate` per R130 perf.
  - **R133 P1-2 wedge-defer preserved**: rollback paths capture
    `should_wedge` and fire `stm_fs_mark_wedged` AFTER unlock — the
...
- **Pre-tip-13**: P9.5-PARALLEL-3 impl-2 (`499a988`). Ports the
  ...
  - Two distinct port patterns from design doc §3.3–3.4:
    - **CREATE-shape** (parent + fresh child): pin parent first,
      ...
      construction — no other writer can resolve to it yet),
      stamp + dirent install. Used by create_file/mkdir/symlink/
      create_anon. The fresh-ino pin can be "out of order" vs
      parent_ino safely because no cycle is reachable.
    - **DELETE-shape** (parent + lookup-discovered child):
      lookup-pin-reverify loop. Dirent lookup gives a candidate
      child_ino; pin (parent, child) in sorted order; re-verify
      the dirent still points to the same child under pin; retry
      on mismatch (FS_UNLINK_MAX_ATTEMPTS = 16 bound). Used by
      unlink/rmdir + link (which is structurally similar — pins
      src_ino+dst_parent, re-verifies the src_parent dirent).
  - Same-(ds, ino) pin_two call refused with STM_EINVAL (would
    deadlock on the ERRORCHECK mutex's double-lock check otherwise).
  - All R128/R130 invariants preserved: pre-cleanup + post-reclaim
    double-commit wired through unchanged; per-inode lock just
    replaces the fs->lock that previously bracketed them.
  - Test fixture fix: impl-1's test_compound_ops_concurrent tests
    were missing `stm_fs_init_dataset_root` after `stm_fs_create_
    dataset` (child datasets don't auto-init their root inode).
    Both impl-1 tests + new impl-2 test (per_inode_create_unlink_
    same_parent_disjoint_names) call it explicitly.
  - New regression test: `per_inode_create_unlink_same_parent_
    disjoint_names` — two writers churn create+unlink under the
    SAME parent dir but with DISJOINT name prefixes. Exercises
    parent-pin contention + child-pin disjoint + dirent_alloc /
    dirent_unlink interleaving + the TOCTOU re-verify loop +
    R128/R130 wiring under SH + pin.
  - **ctest 54/54 GREEN** (test_compound_ops_concurrent now 4 cases:
    original + 2 from impl-1 + 1 new). Rust unit 97/97. e2e_crud
    33/33. concurrent_ctl 2/2.
  - Next: impl-3 ports rename overwrite (4-inode atomicity). impl-4
    ports cross-dataset ops (copy_file_range, reflink). impl-5 drops
...
- **Pre-tip-15**: P9.5-POLISH-1 xattr pair client primitives
  ...
  v9fs mount can now drive getfattr / setfattr / SELinux labels /
  POSIX ACLs via these primitives.
  - `stm_9p_xattrwalk(c, fid, newfid, name, &size)` — server-side
    aux-fid materialization. `name == NULL` ⇒ LIST mode (NUL-
    separated names buffer; Linux listxattr shape). `name`
    non-empty ⇒ VALUE-read mode (named xattr's value). Caller
    drains via `stm_9p_read(newfid)` and clunks the aux fid.
  - `stm_9p_xattrcreate(c, fid, name, attr_size, flags)` — in-
    place fid repurpose into an AUX_XATTR_WRITE collector. Caller
    accumulates `attr_size` bytes via `stm_9p_write(fid)` (append-
    only at the current accumulated offset), then `stm_9p_clunk`
    atomically commits via `stm_fs_setxattr` IFF the size matches.
    Mismatched size discards the buffer (no commit). Commit
    failures surface via Tclunk's Rlerror.
  - Aux-fid lifecycle gotcha (documented in 23-9p_client.md):
    Txattrcreate repurposes the SAME fid slot (no newfid); callers
    that need to keep a NODE binding to the inode MUST Twalk-clone
    the navigation fid FIRST.
  - Trust posture (R111 doctrine carries): lib-boundary refusal of
    `newfid == fid` (mirrors server gate); name length validated
    against STM_9P_NAME_MAX (255); embedded-NUL refused on
    xattrcreate name; strict body-length equality on Rxattrwalk
    (8 bytes) and Rxattrcreate (0 bytes).
  - Tests: 4 new in `tests/test_9p_client.c` — xattrcreate +
    xattrwalk VALUE round-trip (asserts byte-for-byte equality);
    xattrwalk LIST mode after 2 xattrs (asserts NUL-separated
    name buffer is 14 bytes; tolerates either iteration order);
    xattrwalk on a non-existent xattr → STM_ENODATA; lib-boundary
    EINVAL coverage (newfid == fid, NULL out_size, oversize name,
    NULL/empty/oversize xattrcreate name).
  - **ctest 54/54 GREEN** (test_9p_client now runs 62 cases, up
    from 58). Rust suites unchanged.
  - **POLISH-1 #927 is now CLOSED** against the kernel-v9fs
    critical path: Tlock + Tgetlock + Tstatfs + Tsync + Treflink
    + Tfallocate + Tfadvise + Txattrwalk + Txattrcreate are LIVE.
...
| [31-durability-commit.md](reference/31-durability-commit.md) | Durability + the commit path — 2-phase COW commit, barrier ordering, 252-uberblock torn-write defense, the bdev FLUSH (VIRTIO_BLK_F_FLUSH), crash consistency, commit-throughput (Stabilization Area G) | medium |
| [32-decrypted-extent-cache.md](reference/32-decrypted-extent-cache.md) | The #343 decrypted-extent cache (read-path plaintext cache, (paddr,gen)/content_hash keys, invalidation-free) + the crypto/integrity throughput baseline (AEGIS-256/XChaCha20-SIV/BLAKE3/xxh3, the measurement reference) (Stabilization Area E) | medium |
