...
- **P5-4b-ii-β: per-pool lock (R16 F4 discharge) — NEXT.** Add `pthread_rwlock_t` to `stm_pool`. Readers: sync commit pool-read phase. Writers: add/remove/begin_evac/finish_evac/evac_step. Cross-cutting refactor across every sync_commit + every pool mutation. Kept out of α to give R17 a focused scope. Relaxes the caller-serialization contract from P5-4a.
...
**P5-4b+: Per-pool lock + sync integration — now P5-4b-ii-β.** See above. Serializes add/remove/replace/evac against concurrent sync_commit internally so the caller-serialization contract from P5-4a can be relaxed.
