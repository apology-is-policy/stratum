...
**P5-4b: `stm_pool_remove_device`.** Evacuation-capable remove. Pre-condition: `RedundancyPreservedOnRemove` (pool can still satisfy redundancy profile without the removed device; the spec's invariant). Evacuates device's allocated blocks (reads each, rewrites on remaining devices via mirror_write), zeros the device's uberblock, burns the UUID (can't be re-added under same UUID), removes from roster. Evacuation is incremental — pause/resume via a progress cursor checkpointed in the allocator tree (ties into alloc_roots format).
