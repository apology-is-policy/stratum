...
**P5-4c: `stm_pool_replace_device`.** Two paths per ARCH §4.7.3: (a) ONLINE→ONLINE — straight copy from old to new. (b) FAULTED→new — reconstruct via P5-3c's mirror-read from surviving replicas. Atomically swaps roster slots (no cardinality change → no redundancy check).
