...
**P5-4b: `stm_pool_remove_device` — split into two sub-chunks.**
...
- **P5-4b-ii: evacuation half — NEXT.** Iterate the target device's alloc tree; for each allocated range, read + `mirror_write` to survivors, then release the range in the tree. Crash-safety invariant: every block is either on the old device OR on ≥mirror_n surviving replicas, never neither. Likely a spec extension (`evac.tla` or `device_lifecycle.tla` extension) modeling the torn-evacuation state. Per-pool lock (R16 F4 discharge) bundles here — evacuation runs across many commits, and the concurrency surface widens enough to want the lock anyway. Also handles UB-ring zeroing on the removed device and cursor persistence in `alloc_roots` for pause/resume.
...
**P5-4b+: Per-pool lock + sync integration.** Serializes add/remove/replace against concurrent sync_commit internally so the caller-serialization contract from P5-4a can be relaxed.
