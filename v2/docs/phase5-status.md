...
**P5-3b: Per-device allocator trees + alloc-roots object** (ARCH §6.1, the "chosen alternative" at lines 1385-1386). Refactor. Each device gets its own `stm_alloc` tree; the pool-level "allocator-roots object" is a small btree keyed by `device_id` → `(root_paddr, root_csum, root_gen)` bptr, stored in device-0's bootstrap pool (metadata primary). `ub_alloc_root` now points at this roots object instead of directly at a single alloc tree. Merkle chain is transitively complete (ub_alloc_root.bp_csum covers roots-object root node; its leaf values carry per-device alloc-tree root csums). For N=1, the roots object has one entry — extra level of indirection, no behavioral change at the FS layer. `stm_sync` becomes multi-alloc-aware (holds array of `stm_alloc *` + the roots-object handle); commit + open iterate over every device's alloc. Tests: mostly internal refactor; N=1 tests stay green, N=3 multi-device alloc test exercises per-device trees.

**P5-3c: `stm_alloc_reserve_mirror` + write fan-out + read fallback**. Once P5-3b lands, this is additive.
- `stm_alloc_reserve_mirror(pool_alloc, n, out_paddrs[n])` — picks n distinct devices via a placement scorer (simple free-space for MVP; full scorer per ARCH §6.7.2 later), reserves one paddr per device's alloc tree.
- Write path: issue n parallel writes; succeed on ⌊n/2⌋+1 confirmations; fail on less.
- Read path: read from any replica; on AEAD tag / csum failure, fall back to next replica.
- Mirror invariant: every logical block durably stored on n physical paddrs across n devices.
- Crypto: each replica's paddr yields its own nonce (ARCH §4.4.3).
