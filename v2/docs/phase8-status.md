...
| **P8-POSIX-2b** *(substantive landed; R73 audit pending)* | fs.c POSIX wrappers composing dirent + inode at the per-fs lock layer: `stm_fs_lookup` / `_create_file` / `_mkdir` / `_unlink` / `_rmdir`. Lock chain: `fs->lock → inode_idx mutex / dirent_idx mutex` (siblings, no cross-locking). Atomicity: create-paths roll back the freshly-allocated inode if dirent-link fails (no orphans on failure). MVP single-link semantics — unlink frees the inode unconditionally; hard links + nlink decrement defer to P8-POSIX-3. POSIX name validation: 1..255 bytes, no NUL/'/'; "." and ".." reserved. New error codes `STM_ENOTDIR / STM_EISDIR / STM_ENOTEMPTY` (POSIX-aligned values) for dirent-shape refusals. 11 new tests in test_fs.c (lookup empty + create-then-lookup + duplicate-rejection + mkdir + unlink + unlink-refuses-dir + rmdir-refuses-non-empty + rmdir-refuses-file + lookup-refuses-non-dir-parent + arg validation matrix + create-rolls-back-inode-on-EEXIST). | composes over `inode.tla` + `dirent.tla` (no spec extension required for MVP single-link semantics; nlink + hard-link atomicity invariants land at P8-POSIX-3) | composes 2 trigger surfaces: inode allocator + dirent layer + Public API contracts (5 new fs.h decls) |
