...
| **P8-POSIX-2** *(spec scaffold landed)* | Dirent layer. Hash-indexed B-tree per ARCH §11.4: key `(dir_ino, STM_KEY_DIRENT, fnv1a(name) + probe_offset)`. Probe-chain for collisions. `stm_fs_lookup` / `stm_fs_create_file` / `stm_fs_mkdir` / `stm_fs_unlink` / `stm_fs_rmdir`. | **`dirent.tla` LANDED** — open-addressing chain integrity (581 healthy states / depth 7); 3 buggy variants `BuggyUnlinkUsesEmpty` (82 states), `BuggyCreateOverwritesNoProbe` (14 states), `BuggyLookupStopsOnTombstone` (84 states) all fire as designed. | new surface (`src/dirent/` joins CLAUDE.md trigger list at substantive landing) |
