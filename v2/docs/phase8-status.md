...
| **P8-POSIX-3** *(substantive landed; R74 audit pending)* | Metadata ops + nlink + hard links. New fs APIs: `stm_fs_stat` (returns full 256-byte stm_inode_value), `stm_fs_chmod` (preserves S_IFMT bits; rejects type-mismatch), `stm_fs_chown` (UINT32_MAX = leave-unchanged, POSIX `chown(-1)` semantics), `stm_fs_utimens` (atime + mtime + caller-supplied ctime; nsec validated < 1e9), `stm_fs_link` (same-dataset hard link; cross-dataset refused with STM_EXDEV; hard-link-on-directory refused with STM_ENOTSUPPORTED per POSIX). New inode-layer APIs: `stm_inode_link` (nlink++) + `stm_inode_unlink` (nlink--; cascade-free atomically when nlink reaches 0 — models inode.tla::Unlink). The MVP single-link unconditional `stm_inode_free` from P8-POSIX-2b's unlink/rmdir wrappers has been replaced by `stm_inode_unlink`'s cascade-free path — behavior-equivalent for MVP since alloc creates nlink=1, but enables hard-link semantics where unlink-of-one-dirent decrements nlink and KEEPS the inode reachable via the other dirent(s). 7 new tests in test_fs.c (stat / chmod-preserves-type / chown-minus-one / utimens-basic + nsec-validation / link-basic + nlink tracking / link-refuses-directory / link-refuses-existing-dst with rollback). 4 new tests in test_inode.c (link-increments-nlink / unlink-cascade-only-at-zero / link-refuses-freed / arg validation). 37 ctest suites green default. | **`inode.tla` EXTENDED** — adds `nlink` shadow var + Link/Unlink actions + `LinkedAllocatedHasPositiveNlink` + `FreedHasZeroNlink` invariants. New `BuggyUnlinkLeavesZeroNlink` config (fires LinkedAllocatedHasPositiveNlink). Healthy 9.87M states / 2.96M distinct (Inos=3, MaxGen=3, MaxNlink=2). | inode (allocator + new link/unlink) + fs (5 new public APIs) |
