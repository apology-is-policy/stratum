...
| `__P6PERF__` | **P6-perf bench: snap_create + snap_delete benchmarks (ROADMAP §9.2 #1 + #2 confirmation).** New `v2/bench/bench_snapshot.c`; opt-in via `cmake -DSTM_BUILD_BENCHES=ON`. Measures: snap_create per-op latency vs in-pool snap population (10..100k slots); snap_create vs num-of-datasets (slots fixed); snap_delete per-op latency vs dead-list size (0..256); overwrite_block throughput (R33 P2 dup-paddr scan cost). Reference numbers on Apple Silicon M-series 8-core: snap_create at N=100k snaps = 0.91 ms/op, well under the 10 ms criterion; snap_delete is constant-time O(1) at the snap-module layer (caller bears the per-paddr reclaim O(K)). No protocol surface; no audit triggered (CLAUDE.md trigger list explicitly excludes pure instrumentation). Closes ROADMAP §9.2 exit criterion #1; corroborates #2. | No new ctest cases — bench is opt-in, not part of the regular suite. Numbers documented in `v2/bench/README.md`. |
...
Status as of tip `d4efeeb`:

- [x] Snapshot create < 10 ms regardless of dataset size. (Snap
      module never walks the dataset btree; scaling axis is in-pool
      snapshot count, not dataset size. Bench `bench_snapshot` shows
      < 1 ms even at N=100k snapshots — comfortably under 10 ms.)
- [x] Snapshot delete's work proportional to blocks freed, not
      total tree. (P6-deadlist: incremental dead-list maintained on
      COW; the snap-module portion of delete is O(1) — a pointer
      swap that hands the dead_list buffer to the caller. The
      O(dead_count) reclaim work is in the caller's `stm_alloc_free`
      loop. dead_list.tla single-ownership model + bench
      `bench_snapshot::snap_delete_vs_dead_list` confirm.)
...
## Remaining Phase 6 work (as of tip `d4efeeb`)
...
| Dead-list C impl (persistent dead-list bytes + `stm_snapshot_index_overwrite_block` + production-quality `stm_snapshot_delete` returning the freed-paddr list) | **Done at `18b9289` + R33 close `d4efeeb`** | STM_UB_VERSION 10 → 11. Snapshot value tail extends with `le32 dead_count + le64 paddrs[N]`. STM_SNAP_DEAD_LIST_MAX = 256 (in-line MVP; chunked storage for very-large dead-lists deferred). 12 new tests in test_snapshot (post-R33). dead_list.tla single-ownership: surviving = ∅ ⇒ all of S.dead frees on Delete. R33 audit closed clean (0 P0 + 0 P1 + 1 P2 + 5 P3, all addressed). |
...
| Perf bench harness (snap create / delete benchmarks) | **Done** | `v2/bench/bench_snapshot.c`. Snap_create at N=100k snaps: 0.91 ms/op (< 10 ms). Snap_delete: O(1) at the snap-module layer; per-paddr reclaim is the caller's responsibility. Output documented in `v2/bench/README.md`. |
| ROADMAP §9.2 exit criteria | **5/5 met** | See top of this file for status table. All five criteria now met (property inheritance, dataset persistence, structural-clone divergence, snap_create perf, snap_delete-proportional-to-freed). Phase 6's namespace layer is feature-complete. |
