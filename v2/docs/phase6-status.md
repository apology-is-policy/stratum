...
| (this chunk) | P6-3 C impl: snapshot module — in-RAM MVP per snapshot.tla. New `src/snapshot/snapshot.{h,c}` (~480 LOC) + 21 tests. Atomic Create/Delete/Hold/Release under pthread ERRORCHECK mutex; per-dataset chain via prev_snap_id (linked at Create from most_recent); O(n) linear-array storage; persistent storage (ub_snap_root wiring) is a follow-on chunk. R29 audit pending. | 31/31 ctest pass on default + ASan + TSan; test_snapshot 21 cases including TSan-clean concurrent stress (8 threads × 100 creates × different datasets). |
