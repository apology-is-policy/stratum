...
| (this chunk) | P6-2 C impl: dataset module — in-RAM MVP per dataset.tla. New `src/dataset/dataset.{h,c}` (~535 LOC) + 25 tests. Atomic Create/Destroy/Rename/Move under pthread mutex; O(n) linear-array storage; persistent storage (ub_main_root wiring) is a follow-on chunk. R28 audit pending. | 30/30 ctest pass on default + ASan + TSan; test_dataset adds 25 cases covering every dataset.tla invariant + every action precondition. |
