...
| (this chunk) | P6-4 C impl: property API on dataset module — in-RAM MVP per property.tla. Extends `include/stratum/dataset.h` + `src/dataset/dataset.c` (no new module). Three property kinds (INHERITABLE/NONINHERITABLE/IMMUTABLE) covering ARCH §8.4.2 representative properties (compress/quota/encryption). Local override + parent-chain walk + pool default + immutable-set-once semantics. Test_dataset grows 28 → 37 cases. R30 audit pending. | 31/31 ctest pass on default + ASan + TSan. All property.tla invariants exercised end-to-end. |
