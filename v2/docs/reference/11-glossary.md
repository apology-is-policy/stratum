...
| **truncate single-lock-span (P7-11)** | `stm_sync_truncate` post-P7-11 holds `sync->lock` across all three phases (lookup → read+re-encrypt → past-extent drop + drop-route) under one acquisition. Closes **R41 P3-1 case (a)** (concurrent `stm_sync_commit` between Phase 2 and Phase 3 splits the on-disk view) and **R41 P3-2** (same gap, scrub-flavored). **R41 P3-1 case (b)** — Phase 3 `stm_extent_truncate` ENOMEM after Phase 2 succeeded — is NOT closed; the in-RAM prefix-shrink can be persisted by a subsequent `stm_sync_commit`, leaving the file with the prefix shrunk but past extents still present. No load-bearing invariant violated; POSIX-atomicity gap. Operator retries the truncate (idempotent). Trade-off is lock-hold duration: decrypt + encrypt + bdev I/O for the crossing extent's prefix all happen under `sync->lock`; cascade-extends scrub-step latency by the verify cb's brief s->lock takes. POSIX truncate-vs-write atomicity at the FS-layer file-handle level remains the caller's responsibility. |
