...
All 29 fixed configs green (one per module + `scrub_beta` +
...
All 49 buggy configs reproduce their designed invariant
...
data buggy variants: `inode_truncate_reinlines_buggy`
(TruncateExtent reverts to KIND_INLINE — fires
OneWayInlineToExtent), `inode_inline_write_spills_buggy`
(WriteInline allows data_len > MaxInline without transitioning —
fires InlineLenBounded)).
