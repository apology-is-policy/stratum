...
P8-POSIX-2's `dirent.cfg`). All 47 buggy configs reproduce
their designed invariant violations (was 40 → +3 P8-POSIX-2
write-side chain-integrity → +1 P8-POSIX-3 inode_unlink_leaves_zero_nlink
→ +3 P8-POSIX-4 readdir cursor-stability buggy variants:
`dirent_readdir_includes_tombstones_buggy` (tombstone leaks to
emit_log), `dirent_readdir_no_cursor_advance_buggy` (same probe
re-emitted), `dirent_readdir_skips_live_record_buggy` (cursor
advances past live record without emitting)).
