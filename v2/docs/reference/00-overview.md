...
| 7 | 🚧 in progress | P7-prework FastCDC + P7-1 extent.tla spec + P7-2 extent C impl + P7-3 extent persistence (UB v11→v12) + P7-4 fs.c/sync.c COW integration + P7-5 production scrub cb + P7-6 replica-list extension (UB v12→v13) + P7-7 send/recv MVP + P7-8 snap-gen alignment (extent_txg field + UB v13→v14) + P7-9 truncate partial-extent split + P7-10 per-dataset DEKs (extent `key_id` field + UB v14→v15 + sweep refcount enforcement) + P7-11 truncate `_locked` atomicity refactor + P7-12 truncate fault-free Phase 3 (closes R41 P3-1 fully + R41 P3-2; truncate is now atomic + ENOMEM-safe) + P7-13 `stm_fs_create_dataset` (one-step bundle of dataset_index + keyschema provisioning under fs->lock; wrap-key source bound to mount-time path per ARCH §7.7) + P7-14 snap chain-ordering regression test (closes R40 P3-3; gated `_for_test` seam via new `STRATUM_BUILD_TESTING_HOOKS` CMake option). R27, R34–R46 closed. Pending: CAS, reflinks, repair log persistence. | phase7-status.md |
