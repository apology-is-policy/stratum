...
| 7 | 🚧 cold-tier MVP feature-complete + orchestration end-to-end + migration policy v1 + per-dataset opt-in + send/recv cold | P7-prework FastCDC + P7-1..P7-16 + P7-CAS index foundation (UB v17→v18) + P7-CAS-2 migration data plane + P7-CAS-3 R50 P2-1/P2-3 close + cold-extent reflink + P7-CAS-4a crossing-cold truncate + P7-CAS-4b FastCDC sub-chunking + P7-CAS-4c snap_idx ↔ CAS hash refcount integration (UB v18→v19; cold-dead-list multiset; `stm_snapshot_index_overwrite_cold_block` + extended `stm_snapshot_delete`) + P7-CAS-4 background-GC semantics (cas_auto_gc_sweep_locked reorder cas_gc-then-alloc_free; FAULTED/REMOVED skip; cold-dead-list capacity pre-check via `stm_snapshot_index_cold_dead_list_reserve`; closes R51 P3-2/P3-4 + R54 P3-1/P3-2/P3-3) + P7-CAS-5 out-of-band CAS GC entry point + P7-CAS-6 scrub-orchestrator wrapper + P7-CAS-7 migration-policy heuristic v1 (`stm_fs_migrate_policy_step` driving age-based candidate selection over a single dataset) + P7-CAS-8 per-dataset `STM_PROP_TIERING` opt-in + multi-dataset wrapper `stm_fs_migrate_policy_pass_all` (UB v19→v20; STM_PROP_COUNT 3 → 4; INHERITABLE tiering preference walks parent chain) + P7-CAS-9 send/recv with cold extents (`STM_SEND_FLAG_COLD` wire-format extension + `stm_sync_recv_cold_extent` receiver primitive; storage-at-rest dedup preserved on target). R27, R34–R60 closed. Recordsize lift (cross-extent FastCDC), promotion (cold→hot) heuristic, and learned migration policy v2 deferred. | phase7-status.md |
