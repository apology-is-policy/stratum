...
| 20 → 21 | P7-CAS-11: extent record value layout 96 → 108 — appends `read_count` (le32 at offset 96..100) + `last_read_gen` (le64 at offset 100..108) for the promotion (cold→hot) heuristic. The counter is COLD-only — HOT extents have both fields == 0 with decoder anti-tamper enforcement. Windowed-count semantics (compile-time default `STM_SYNC_PROMOTE_DECAY_WINDOW_DEFAULT_TXGS = 1024`, renamed at v22 from `STM_SYNC_PROMOTE_DECAY_WINDOW_TXGS`; per-dataset override at v22 via `STM_PROP_PROMOTE_DECAY_WINDOW`): each COLD read saturating-increments via `stm_extent_record_promote_read_hit`; reads more than `decay_window` txgs after the previous read reset the counter to 1. v20 pools' 96-byte extent values length-rejected by v21 decoder; refused via uniform STM_EBADVERSION at SB version check. New extent-tree APIs: `stm_extent_promote_swap_to_hot` (atomic cold→hot, returns dropped hash) + `stm_extent_record_promote_read_hit` (windowed-count mutate). New sync APIs: `stm_sync_promote_to_hot` (per-ino driver) + `stm_sync_promote_policy_collect` (heuristic candidate collector). New fs APIs: `stm_fs_promote_to_hot` + `stm_fs_promote_policy_step` + `stm_fs_promote_policy_pass_all`. Composition over `cas.tla::RehydrateOnWrite` — the cold→hot transition is identical to the existing auto-rehydrate-on-write path; only the trigger differs (frequent reads vs overlapping write). **No spec extension required.** No uberblock field changes.

| 21 → 22 | P7-CAS-12: STM_PROP_COUNT 4 → 5 — adds `STM_PROP_PROMOTE_DECAY_WINDOW` (INHERITABLE; per-dataset override for the per-COLD read-frequency counter's decay window in txgs). Effective value 0 = use compile-time default (`STM_SYNC_PROMOTE_DECAY_WINDOW_DEFAULT_TXGS = 1024`); non-zero = use that window. The bump call site at `stm_sync_read_extent_locked`'s COLD branch resolves the effective property at each successful decrypt and passes the result to `stm_extent_record_promote_read_hit`. Lock order is preserved: sync->lock (held) → dataset_idx mutex (acquired here); dataset.c never calls back into sync. Dataset value layout grows past the local_value[] tail: `origin_snap_id` moves from offset 64 to offset 72 (= `32 + 8 * STM_PROP_COUNT`); `DS_VAL_FIXED` grows from 72 to 80. Pool-defaults value length grows from 32 to 40 bytes. The local_set bitmap (le16 at offset 28..29) gains a 5th significant bit; bitmap-mask validation rejects bits beyond `STM_PROP_COUNT - 1`. v21 pools refused at v22 mount via uniform STM_EBADVERSION (no in-place forward-compat — same posture as v19→v20). No uberblock field changes. property.tla unchanged (parametric over Properties; existing INHERITABLE-class invariants already cover the new property). cas.tla unchanged (the bump logic is heuristic state, not load-bearing). **No spec extension required.**

| 22 → 23 | P7-CAS-16: recordsize cap lift `128 KiB → 8 MiB` — `STM_FS_RECORDSIZE_MAX` (the runtime invariant on extent record `len`) lifts from `128 * 1024` to `8 * 1024 * 1024`. The cap is enforced at write entry (`stm_sync_write_extent`), at every extent-record decode + load_validate site (sync.c read / migrate / promote / recv paths), and at the wire-format CHUNK / HOT-EXTENT body cap (`STM_SEND_CHUNK_PLAIN_MAX`). The on-disk extent encoding's 24-bit `dlen` / `clen_and_comp.clen` slot (`EX_LEN_MAX_24BIT = 0x00FFFFFF = 16 MiB - 1`) accommodates 8 MiB without an encoding-shape change. `STM_FS_RECORDSIZE_MAX` was promoted from a private define in `src/sync/sync.c` to `include/stratum/fs.h` so call-site invariants and tests can reference the constant directly. v22 pools' extent records are forward-compatible at the value level (`len ≤ 128 KiB ≤ 8 MiB`) but rejected up-front at SB version check via uniform STM_EBADVERSION; the inverse direction (v23 → v22) is unsafe because v23 may have written `128 KiB < len ≤ 8 MiB` extents that v22 binaries' load_validate would reject as oversized (`STM_ECORRUPT`). Lockstep wire-format bump: `STM_SEND_VERSION 2 → 3` (see send_recv.h). No uberblock field changes. **No spec extension required** — the cap is a parametric bound, not a load-bearing protocol invariant; cas.tla / extent.tla / dead_list.tla / property.tla all unchanged.

| 23 → 24 | P8-POSIX-1b: per-pool inode tree on-disk surface (ARCH §11.3 / inode.tla). Two new fields carved from the head of `ub_reserved`: `ub_inode_root` (64-byte stm_bptr at offset 3288) + `ub_inode_root_gen` (le64 at offset 3352); `ub_reserved` shrinks 776 → 704 bytes. Anchors the per-pool inode-index Bε-tree keyed by `(le64 dataset_id || le64 ino) → 256-byte stm_inode_value` per ARCH §11.3 (with v2 simplification: 16-byte (paddr, gen) extent-tree-root vs ARCH's 57-byte stm_bptr). New `STM_BPTR_KIND_INODE_TREE = 12`. R70 P0-1: the Merkle root chain folds in an 8th csum (`inode_csum`) — closes the asymmetry where adding a load-bearing tree-root field without binding it into the Merkle chain would let an offline-write attacker swap the inode tree (e.g., point at a stale snapshot at a recorded paddr/csum) by recomputing `ub_csum` alone. With the binding, any tamper to the inode tree forces the Merkle root to mismatch at `sync_open`'s recompute. Same fix-shape as R47 P2-1 for `repair_log_csum` at v16. AllocReused gen-bump persistence per `inode.tla::AllocReused`: FREED state encoded inline via `STM_INO_FLAG_FREED` in `si_flags`. v23 pools refused at v24 mount via uniform STM_EBADVERSION (no inode tree on v23 + Merkle binding mismatch even if forwarded). No wire-format change. **No spec extension required** — `inode.tla` already covered the AllocReused state machine at P8-POSIX-1; persistence composes via `btree_store` (existing pattern). |

| 24 → 25 | P8-POSIX-2: per-pool dirent tree on-disk surface (ARCH §11.4 / dirent.tla). Two new fields carved from the head of `ub_reserved`: `ub_dirent_root` (64-byte stm_bptr at offset 3360) + `ub_dirent_root_gen` (le64 at offset 3424); `ub_reserved` shrinks 704 → 632 bytes. Anchors the per-pool dirent-index Bε-tree keyed by `(le64 dataset_id || le64 dir_ino || le64 hash_probe)` (24 bytes) where `hash_probe = fnv1a64(name) + probe_offset` per ARCH §11.4.1 + dirent.tla. Values are variable-length 32 + name_len bytes (32-byte fixed prefix + 1..255 byte name). Open-addressing chain integrity per dirent.tla: tombstones encoded inline via `STM_DIRENT_FLAG_TOMBSTONE` in the value's flags byte; lookup walks past tombstones; unlink leaves a tombstone (NOT empty) to preserve reachability of colliding names at higher probe indices; create installs at the first EMPTY-or-TOMBSTONE slot in the chain. New `STM_BPTR_KIND_DIRENT_TREE = 13`. The Merkle root chain folds in a 9th csum (`dirent_csum`) in lockstep with this field's introduction (R70 P0-1 + R71 P1-1 lessons — every load-bearing tree-root field MUST bind into the Merkle chain in the same commit, and writer-side guards must mirror decoder-side guards). v24 pools refused at v25 mount via uniform STM_EBADVERSION. Probe cap STM_DIRENT_PROBE_MAX = 64 per ARCH §11.4.2. No wire-format change. **No spec extension required** — `dirent.tla` already covered the open-addressing state machine at P8-POSIX-2 spec scaffold; persistence composes via `btree_store`. New audit-trigger surface: `src/dirent/` + `include/stratum/dirent.h` join the CLAUDE.md trigger list. |

| 25 → 26 | P8-POSIX-6: per-pool xattr tree on-disk surface (ARCH §11.5 / xattr.tla). Two new fields carved from the head of `ub_reserved`: `ub_xattr_root` (64-byte stm_bptr at offset 3432) + `ub_xattr_root_gen` (le64 at offset 3496); `ub_reserved` shrinks 632 → 560 bytes. Anchors the per-pool xattr-index Bε-tree keyed by `(le64 dataset_id || le64 ino || le64 hash_probe)` (24 bytes) where `hash_probe = fnv1a64(name) + probe_offset` per ARCH §11.5 + xattr.tla — structurally isomorphic to the dirent tree's key shape, only rooted at `ino` instead of `dir_ino`. Values are variable-length 16 + name_len + value_len bytes (16-byte fixed header + 1..255 byte name + 0..65536 byte value); tombstones are 16 bytes. Open-addressing chain integrity per xattr.tla — same three rules as dirent.tla, only renamed Set/Remove (POSIX vocabulary): Remove leaves a TOMBSTONE; Set walks the chain to find first EMPTY-or-TOMBSTONE slot; Get walks past tombstones. New `STM_BPTR_KIND_XATTR_TREE = 14`. The Merkle root chain folds in a 10th csum (`xattr_csum`) in lockstep with this field's introduction (R70 P0-1 + R72 + R77 P1-1 lessons — Merkle binding closes the offline-write tampering surface AND a writer-side guard symmetry on `value_len ≤ STM_XATTR_VALUE_MAX = 64 KiB` closes the analog of R77 P1-1's OOB-read shape that extended from inline data to xattr values). v25 pools refused at v26 mount via uniform STM_EBADVERSION. Probe cap STM_XATTR_PROBE_MAX = 64 (same as STM_DIRENT_PROBE_MAX, independent macro for future tuning). No wire-format change. **No spec extension required** — `xattr.tla` covers the open-addressing state machine; persistence composes via `btree_store`; POSIX namespace gating (`user.` / `system.` / `security.` / `trusted.`) is fs.c-layer policy, not a load-bearing chain-integrity invariant. New audit-trigger surface: `src/xattr/` + `include/stratum/xattr.h` join the CLAUDE.md trigger list. |

| 26 → 27 | TLY-A3-keyslot: corvus `wrapper_identity` tag on keyschema entries. **No uberblock field change** — this bump versions the keyschema sub-tree entry-value semantics. Byte [2] of each keyschema entry value (`state(1) || flags(1) || wrapper_identity(1) || reserved(5) || wrapped`) was previously part of a 6-byte reserved block written zero; it now carries `wrapper_identity` ∈ {LEGACY=0, PASSPHRASE=1, JANUS=2, CORVUS=3} (see `keyschema.h`). The on-disk layout is byte-identical (`KS_VAL_HDR_LEN` stays 8; reserved → meaningful) so the change is forward/backward COMPATIBLE at the byte level — a v27 binary reads a v26 pool's keyschema as all-LEGACY. The bump exists for the SEMANTIC break: a pre-TLY-A3 binary has no `wrapper_identity` concept and would misroute a CORVUS-wrapped keyslot through its keyfile/janus path. The exact-match SB version check makes a v26 binary refuse a v27 pool outright — strictly stronger than an incompat feature flag, and the only refusal mechanism the v2 tree uses (`ub_flags_*` remain vestigial). No wire-format change. **No spec extension to a NEW file** — `key_schema.tla` is extended in-place with `WrapperConsistent` + `MountResolvesKeyBeforeData` + a `wrapper_identity` non-perturbation check (TLY-A3-keyslot-spec). Precedent for a value-format-driven bump rather than a UB-field bump: v17 → v18. See `v2/docs/thylacine-keyslot-design.md`. |

| 27 → 28 | TLY-A3-keyslot-wrap: corvus dataset-path binding on keyschema entries. **No uberblock field change** — this bump versions the keyschema entry-value LAYOUT. Byte [3] of each keyschema entry value (`state(1) || flags(1) || wrapper_identity(1) || corvus_path_len(1) || reserved(4) || corvus_dataset_path || wrapped`) was previously reserved-zero; it now carries `corvus_dataset_path_len`, and a variable-length corvus dataset-path (1..255 bytes) is stored immediately after the 8-byte header, BEFORE the wrapped blob. A `CORVUS` slot records the UTF-8 dataset path corvus binds into the DEK envelope's AEAD-AD (`STRATUM-API-V1.md` §5.10) — the mount-time UNWRAP sends it back, so a wrong identifier makes corvus reject the envelope (`key_schema.tla::UnwrapUsesWrapBinding`). Every other wrapper records a zero-length path; `insert_wrapped`/`rotate` enforce the CORVUS⟺path rule (`STM_EINVAL`), `decode_val` re-checks it (`STM_ECORRUPT`). **Unlike the 26→27 bump this is a genuine LAYOUT change** — the wrapped blob shifts by `corvus_path_len` bytes — so a v27 binary would mis-slice a v28 value; the exact-match SB version check makes a v27 binary refuse a v28 pool outright. v27 pools refused at v28 mount via uniform STM_EBADVERSION. No wire-format change. `key_schema.tla` extended in-place with `UnwrapUsesWrapBinding` + the `unwrap_path` tracking variable (TLY-A3-keyslot-wrap-spec). See `v2/docs/thylacine-keyslot-wrap-design.md`. |

| 28 → 29 | 9.6-impl-4d: cut the extent index over to the `btree_engine` substrate (the same persistence layer the inode / dirent / xattr indices use as of 9.6-impl-4b/c). **No uberblock field change** — `ub_extent_root` / `ub_extent_root_gen` are stamped exactly as before. The on-disk key + value layout for individual extent records is UNCHANGED (24-byte key, 108-byte value v21). What changes is the on-disk envelope at `ub_extent_root.bp_paddr`: the prior `btree_store` whole-tree-rebuild MVP becomes the engine's incremental-COW B+tree (16 KiB nodes, AEAD-encrypted via the shared crypt ctx, per-node BLAKE3 Merkle csum + deferred-free + three-phase commit). A v28 binary would mis-read a v29 extent tree (and vice-versa) because the root envelope's serialised form differs; the exact-match SB version check makes the cross-version mismatch refuse-loud. v28 pools refused at v29 mount via uniform STM_EBADVERSION. The sync layer's commit cascade extends from three engine-backed flushes (inode + dirent + xattr) to four (extent + inode + dirent + xattr); the abort cascade extends in lockstep so every error path between any flush and the post-UB finalize aborts the right subset of pending engines (extent flushes first; the cascade matrix is in `reference/07-sb-sync.md`). The extent module's monolithic `_commit` retains its internal `stm_bootstrap_commit` call for test-only isolation; the sync path uses `_commit_flush` / `_finalize` / `_abort` and the explicit `stm_bootstrap_commit` at sync.c is now the SOLE durable-bitmap barrier for the four engine-backed indices. No wire-format change. **No spec extension required** — `extent.tla` already covers the action-level state machine; persistence composes via the engine's already-verified `btree.tla` invariants (incremental-COW commit, deferred-free, crash-revert, Merkle linkage). See `v2/docs/phase-9.6-impl-4-cutover-design.md`. |

| 29 → 30 | 9.7-impl-1b (Phase 9.7 D1 substrate, foundation chunk): `stm_dataset_entry` gains a 48-byte per-dataset metadata-tree root triple (`di_tree_root` (le64) + `di_root_gen` (le64) + `di_root_csum[32]` BLAKE3) appended after `origin_snap_id`. The dataset entry's encoded length grows from 80 + name_len bytes to 128 + name_len. **No uberblock field change at this bump** — the four pool-global metadata-tree roots (`ub_inode_root` / `ub_dirent_root` / `ub_xattr_root` / `ub_extent_root`) stay live and authoritative at v30; 9.7-impl-1c is the chunk that retires them in favour of the per-dataset triple recorded on each dataset entry. v29 binaries would mis-read a v30 dataset record (the trailing-name offset shifted), so the exact-match SB version check (`version != STM_UB_VERSION` → STM_EBADVERSION) makes a v29 binary refuse a v30 pool outright. v29 pools refused at v30 mount via STM_EBADVERSION. The triple's all-zero state is the "empty dataset" sentinel; fresh datasets persist zeros until 9.7-impl-1c's per-dataset engine produces a real root. See `v2/docs/phase-9.7-design.md` §3.1 / §3.3. **`stm_metakey` lib** lands at 9.7-impl-1a as the foundation for 1c's key-shape migration (single chokepoint for the 1-byte type-tag prefix). |

Current: `STM_UB_VERSION == 30` (see `include/stratum/super.h`).
