...
| 7 | 🚧 cold-tier MVP feature-complete | P7-prework FastCDC + P7-1..P7-16 + P7-CAS index foundation (UB v17→v18) + P7-CAS-2 migration data plane + P7-CAS-3 R50 P2-1/P2-3 close + cold-extent reflink + P7-CAS-4a crossing-cold truncate + P7-CAS-4b FastCDC sub-chunking + P7-CAS-4c snap_idx ↔ CAS hash refcount integration (UB v18→v19; cold-dead-list multiset; `stm_snapshot_index_overwrite_cold_block` + extended `stm_snapshot_delete`) + P7-CAS-4 background-GC semantics (cas_auto_gc_sweep_locked reorder cas_gc-then-alloc_free; FAULTED/REMOVED skip; cold-dead-list capacity pre-check via `stm_snapshot_index_cold_dead_list_reserve`; closes R51 P3-2/P3-4 + R54 P3-1/P3-2/P3-3). R27, R34–R55 closed. Background scrub-β-cb invocation forward-noted (lock-graph rework needed). Migration policy heuristic (NOVEL #6) and recordsize lift (cross-extent FastCDC) deferred. | phase7-status.md |
