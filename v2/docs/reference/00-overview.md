...
Current: `STM_UB_VERSION == 18` (see `include/stratum/super.h`).
...
| 7 | 🚧 in progress | P7-prework FastCDC + P7-1 extent.tla spec + P7-2 extent C impl + P7-3 extent persistence (UB v11→v12) + P7-4 fs.c/sync.c COW integration + P7-5 production scrub cb + P7-6 replica-list extension (UB v12→v13) + P7-7 send/recv MVP + P7-8 snap-gen alignment (extent_txg field + UB v13→v14) + P7-9 truncate partial-extent split + P7-10 per-dataset DEKs (extent `key_id` field + UB v14→v15 + sweep refcount enforcement) + P7-11 truncate `_locked` atomicity refactor + P7-12 truncate fault-free Phase 3 (closes R41 P3-1 fully + R41 P3-2; truncate is now atomic + ENOMEM-safe) + P7-13 `stm_fs_create_dataset` (one-step bundle of dataset_index + keyschema provisioning under fs->lock; wrap-key source bound to mount-time path per ARCH §7.7) + P7-14 snap chain-ordering regression test (closes R40 P3-3; gated `_for_test` seam via new `STRATUM_BUILD_TESTING_HOOKS` CMake option) + P7-15 repair-log persistence (closes R38 P3-1; new src/repair_log/ module + UB v15→v16 with `ub_repair_log_root` / `ub_repair_log_root_gen` / `ub_repair_log_next_seq`; scrub β cb emits per-rewrite audit trail; Merkle root chains repair_log_csum so tamper fires at mount) + P7-16 reflinks (ARCH §11.12; UB v16→v17 with origin-stamping in extent records; SharedReplicasAreCohabit replaces LiveReplicasDisjoint; new `stm_fs_reflink` POSIX-shape FICLONE; cross-dataset deferred via STM_EXDEV per ARCH §11.12.3 same-key requirement) + P7-CAS cold-tier index foundation (ARCH §6.9 / NOVEL #3; UB v17→v18 with `ub_cas_index_root_gen` carved from `ub_reserved`; new src/cas/ module + cas.tla spec + extent-kind discriminator at value byte 0) + P7-CAS-2 migration / rehydrate / auto-GC data plane (new `stm_fs_migrate_to_cold` + atomic `stm_extent_migrate_to_cold` swap + auto-rehydrate on write + COLD-aware read + auto-GC sweep at sync_commit closing R49 P2-1/P2-2; cold-extent reflink + cold-crossing truncate refused with STM_ENOTSUPPORTED in MVP) + P7-CAS-3 closes R50 P2-1 (auto-GC reorder before alloc_commit; PENDING entries persist + sweep at next gen) + R50 P2-3 (transactional 3-phase sweep with idempotent-tolerant retry) + cold-extent reflink (`stm_sync_reflink` Phase 2 / Phase 3 branch on `e->kind`; HOT bumps allocator refcount, COLD bumps CAS refcount via `stm_cas_ref`; rollback symmetrically undoes via alloc_free / cas_deref) + P7-CAS-4a crossing-cold truncate (lifts STM_ENOTSUPPORTED via composition of cold-aware read + hot-encrypt write + cold_overlap_cb pre-scan/post-deref bookend) + P7-CAS-4b FastCDC sub-chunking (integrates `src/cdc/` P7-prework into migration data plane via new atomic `stm_extent_migrate_to_cold_chunked` 1-drop+N-insert primitive; new per-stm_sync `stm_cdc cdc;` field + `<stratum/sync_testing.h>` test-only params override; cas.tla extended with `ChunkedMigrateToColdK2` action + closes pre-existing MigrateToCold clamp/invariant inconsistency via new refcount-cap precondition). R27, R34–R52 closed. Pending: P7-CAS-4b R53 close. | phase7-status.md |
