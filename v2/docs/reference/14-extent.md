...
| `test_fs` | 184 | The fs-integration suite (cross-referenced here for the data-path coverage it exercises via the extent index). Extent-relevant: write/read 4 KiB roundtrip; read-hole returns zeros; write-args validation; COW-drop routing (to alloc-free without a snapshot, to the snap dead-list with one); cross-mount durability; multi-extent per ino; **9.7-impl-4c** `fs_rollback_reclaims_diverged_data_extents` (rollback reclaims the post-snapshot data-extent divergence — `data_allocated_blocks` returns to the post-snapshot value; a heavy post-rollback data workload reissues the freed paddrs without disturbing the snapshot-era data); **9.7-impl-4c-ii** `fs_rollback_reclaims_diverged_cold_extents` (rollback derefs the post-snapshot COLD-extent divergence — CAS entry count returns to the post-snapshot value, and a dedup-shared diverged extent's hash is dereffed EXACTLY once, leaving the snapshot's refcount intact); **9.7-impl-4c-iii** `fs_rollback_reclaims_cleared_dead_list_cold_garbage` (rollback reclaims the snapshot's cleared dead-list COLD garbage — pre-rollback CAS count = 3 entries (case-(a) `H1` from in-snap truncate + case-(b) `H20`, `H21` from post-snap fresh inodes' truncates) → post-rollback CAS count = 1 (only `H1` remains; `H20`, `H21` auto-GC'd via `dead_S(H) − snap_unique(H) = 1` deref each). The case-(a) `H1` chunk is readable post-rollback at refcount 1, proving the snap_unique adjustment correctly cancelled the dead_S(H1) deref. Heavy post-rollback cold-write workload reissues the reclaimed paddrs without disturbing the snapshot-era data; durable across remount). Per-test detail lives in the fs / snapshot reference sections. |
