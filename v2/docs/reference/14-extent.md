...
| `test_fs` | 191 | The fs-integration suite (cross-referenced here for the data-path coverage it exercises via the extent index). Extent-relevant: write/read 4 KiB roundtrip; read-hole returns zeros; write-args validation; COW-drop routing (to alloc-free without a snapshot, to the snap dead-list with one); cross-mount durability; multi-extent per ino; **9.7-impl-4c** `fs_rollback_reclaims_diverged_data_extents`; **9.7-impl-4c-ii** `fs_rollback_reclaims_diverged_cold_extents`; **9.7-impl-4c-iii** `fs_rollback_reclaims_cleared_dead_list_cold_garbage`; **9.7-impl-4d (this chunk)** lifts the impl-4 newer-snapshot refusal and adds 6 new tests: `fs_rollback_destroys_newer_snapshot_cascade` (rolling back past a newer snap now succeeds + the newer snap is destroyed — replaces the prior `..._refuses_when_newer_snapshot_exists` which pinned the impl-4 STM_ENOTSUPPORTED behavior); `fs_rollback_refuses_when_newer_snapshot_held` (R160 P1-1 — pre-drain `STM_EBUSY` refusal as a true no-op; both snapshots remain PRESENT post-refusal); `fs_rollback_4d_frees_newer_snap_case_b_data_garbage` (case-(b) HOT-extent garbage in a newer snap's data dead-list IS reclaimed at rollback); `fs_rollback_4d_keeps_case_a_data_paddrs_live` (case-(a) — a paddr in s.view that ALSO appears in a newer snap's data dead-list stays allocated; the post-rollback live tree reads through it); `fs_rollback_4d_destroys_multiple_newer_snaps` (a chain of 3 newer snaps → all ABSENT post-rollback, target is most-recent); `fs_rollback_4d_cold_dedup_correct` (cold-tier counted subtraction — a hash unique to the newer snap's dead-list dereffes correctly; the case-(a) hash in s.view stays at refcount 1). API-surface tests: `snapshot_collect_newer_empty_returns_ok_null` + `snapshot_collect_newer_returns_ascending_present` (ascending order, only PRESENT snaps, ABSENT slots skipped, args validation). Per-test detail lives in the fs / snapshot reference sections. |
