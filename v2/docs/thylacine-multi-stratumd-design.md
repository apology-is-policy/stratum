...
| **TLY-A2-impl-2** | Client mode: `--role client --coordinator-socket <path>`, raw-frame proxy module (`v2/src/cmd/stratumd/proxy_9p.{h,c}`), Tversion + Tattach interception, `--datasets-allowed` pattern matcher (`v2/src/cmd/stratumd/dataset_pattern.{h,c}`). Tests: fake-coordinator end-to-end with both layers enforcing. | R140 |
| **TLY-A2-impl-3** | Authentication: SO_PEERCRED on downstream side; coordinator validates dialing uid against `--user-policy`; refuse with Rerror(EACCES) on mismatch. Tests: cross-uid Tattach attempts. | R141 |
