...
| `95695f7` | **R11 audit fixes** (post-P4-4b). 0 P0, 2 P1, 4 P2, 5 of 8 P3 closed. P1-1 `janus_listen_unix` chmod failure now fatal (closes the capability bypass where a non-perm-carrying fs silently left the socket world-accessible). P1-2 peer-cred enforced in the accept loop against `geteuid()`; non-matching peers are dropped with an audit log entry. P2-1 underflow-safe `msg_len < 11` guard in client do_read. P2-2 double-close race between main thread cleanup and `on_sigterm`. P2-3 `_Static_assert` tying wrapped-blob + session buffer sizes. P2-4 30s SO_RCV/SNDTIMEO on accepted fds (idle-hold DoS). P3-1 `PASS_MAGIC` fixed to actually spell 'JPAS' on disk. P3-3 server-level open-mode gate in h_read/h_write. P3-4 audit-log truncation now ends in '\n'. P3-5 client iobuf wipes DEK bytes per-read. P3-2/P3-7/P3-8 triaged and documented. 2 new test_janus regressions (`janus_listen_unix_enforces_0600`, `janus_peer_uid_reports_self_on_socketpair`); cross-UID rejection + mock-daemon short-Rread + session-exhaustion tests flagged as tech debt. | 25 tests (9 janus + 16 prior janus-adjacent). 23/23 suites green on default / ASan / TSan. 8/8 TLA+ specs clean. |
...
| `555fcc9` | **R12 audit fixes** (post-P4-4c). 0 P0, 3 P1, 4 of 5 P2, 4 of 6 P3 closed. P1-1 `sync_dek_grow` replaced `realloc` (which could release plaintext-DEK-bearing memory unwiped) with hand-rolled malloc→memcpy→ct_memzero(old)→free. P1-2 `stm_sync_add_dataset_key` OOM rollback was a silent no-op (called `mark_pruning` on a CURRENT entry, which returns STM_EINVAL) — fixed by pre-reserving the DEK-map slot before any schema mutation so the final insert is OOM-infallible. P1-3 RETIRED-key tamper no longer DoSes mount: `sync_unwrap_cb` soft-skips non-`(0, 0, CURRENT)` unwrap failures (retired keys surface at Phase 6 extent read time, not mount time). P2-1 same pre-grow pattern applied to `stm_sync_rotate_dataset_key`. P2-3 added `STM_SYNC_DATASET_ID_MAX = 0x0FFFFFFF` mirroring janus qid width; both add + rotate APIs return STM_ERANGE above the cap. P2-4 `stm_keyschema_insert_wrapped` narrowed to CURRENT-only (RETIRED/PRUNING go through the state-enforcing primitives). P2-5 `sweep_collect_cb` error precedence flipped so `col.err` wins over iter's return. P3-1 two `memset`→`stm_ct_memzero` wipes in keyschema.c. P3-3 `stm_janus_client_rotate` restructured to single `read_done:` cleanup path. P3-4 `parse_dataset_id` cap tightened 12→9 digits. P3-5 `janus_synfs_drop_all_sessions` called at connection teardown so rotate / unwrap sessions left un-clunked have their DEK-bearing resp_bufs wiped on client disconnect. Triaged: P2-2 (daemon HWM — requires stateful daemon), P3-2 (dek_count NULL vs empty — doc-only), P3-6 (atomic-point comment). 4 new regression tests in `test_keyschema_rotate` (15 total). | 24/24 suites green on default / ASan / TSan. 8/8 TLA+ specs clean. |
| `5a50df0` | **P4-4c: per-dataset keys + rotation + retired-key sweeper** (ARCH §7.7.2). New keyschema primitives: `stm_keyschema_next_key_id` / `_rotate` (atomic: insert new CURRENT + retire old) / `_mark_pruning` / `_prune` / `_iter`. New sync-layer APIs: `stm_sync_add_dataset_key` (for ds ≥ 1; ds 0 is installed by `sync_create`), `stm_sync_rotate_dataset_key` (rejects ds 0 as STM_EBUSY — metadata-key rotation requires a not-yet-implemented re-encrypt sweep), `stm_sync_keyschema_sweep` (RETIRED → PRUNING → deleted, safe in Phase 4 because no extent layer references these keys), `stm_sync_get_dek`, `stm_sync_dek_count`. Sync handle now carries a `(dataset_id, key_id) → 32B DEK` map populated on `sync_open` by iterating every CURRENT+RETIRED schema entry and unwrapping via keyfile or janus. Janus gains a `/pools/<uuid>/datasets/<id>/rotate` node (ORDWR): client writes `new_key_id` (u64 LE), reads `DEK(32) ‖ wrapped`; both file and passphrase backends implement the new `wrap` vtable entry. Synfs drops the single-dataset-per-pool constraint — any numeric `dataset_id` is walkable, with `dataset_id` packed directly into the lower 28 bits of `qid_path`. `janus_synfs_register_pool` signature change: `dataset_id` param removed. `key_schema.cfg` widened from `Datasets={0}` back to `Datasets={0,1}`; TLC clean (8649 distinct states, depth 17). Fixes pre-existing sync-commit bug (R7c P2-5 skips tree rewrite when clean, but `sync_commit` wrote `alloc_root_gen=commit_gen` unconditionally → STM_EBADTAG on reopen after two commits with an intermediate schema-only change). New `stm_alloc_get_tree_gen` returns the ACTUAL last-rewritten gen; sync_commit threads it into ub_alloc_root_gen. | 11 new tests in `test_keyschema_rotate` (primitives, keyfile-rotate, janus-rotate, ds=0 refused, add ds=0 refused, cross-dataset isolation, sweep removes retired, sweep preserves CURRENT, rotation persists across mount, swept keys don't reappear, monotonic key_ids across sweep). 24/24 suites green on default / ASan / TSan. 8/8 TLA+ specs clean. |
...
2. **P4-4b — janus daemon (LANDED at `1ae4374`)**. Separate binary,
   ...
   to post-Phase-4.
3. **P4-4c — per-dataset keys (LANDED at `5a50df0`)**. Generalizes
   from one pool key to the dataset-keyed sub-tree + full ARCH §7.7.2
   rotation plumbing + retired-key sweeper. Dataset 0 rotation stays
   gated (STM_EBUSY) pending the metadata re-encrypt sweep; every
   other dataset rotates freely. Sync-handle DEK map hydrated at
   open-time by iterating the schema and unwrapping each entry, so
   readers of data encrypted under retired keys work until the
   sweeper prunes them. Janus gets a `/rotate` synfs node so
   janus-backed pools don't need the FS to hold `hybrid_pk`; the
   daemon is the sole source of DEKs per ARCH §7.7.2 step 1.
