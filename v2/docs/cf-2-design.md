...
In the Thylacine VM (negotiated msize 32 KiB): <= 1 MiB queued (the
budget floor; 4 x 32 KiB = 128 KiB sits below it) + 128 KiB resp per
connection (R176 F3 correction — the earlier 2 MiB figure overstated;
the enforced bound is tighter). A host client that negotiates 8 MiB opts into
...
| CF-2f **BUILT** | bench (gofmt/fsbench A/B re-measure, pool workers=4 vs --fs-workers=1 from an identical seed-restored pool) + docs (29-concurrency.md full as-built rewrite: the two-layer model / pool / 3-phase server / lock hierarchy / 9.8-BE engine / EBUSY retry policy / bench data) + the focused Fable-5-max audit **R176** over the whole 2a-2d surface (CF2-I1..I8 + the #354-class S12 srvconn re-verification + the R174/R175-carried obligations; static-only). Bench findings tracked: #367 (gofmt cold 26.9/27.6s vs the 21.8s pre-CF-1 bar in BOTH pool modes, kernel byte-identical -> CF-1-chunks-8-11-shaped) + #368 (the pool pays 15-24% per-op handoff on single-in-flight paths [fsbench reread -24%, create -18%] and buys nothing on gofmt at -p 4 -> find the real serializer); warm best-ever 3.5-3.7s. The seed twin was refreshed with the #364-fixed cmd/go (the stale twin would have fired the 24h trim with the pre-fix binary -> exit 1 on every future smp-gate restore; landmine killed). | **R176 CONVERGED CLEAN in one round: 0 P0 / 0 P1 / 0 P2 / 3 P3** (MODEL start==end Fable 5; concurrent self-audit traced 6 surfaces independently — monotone callback, concurrent-flag threading, budget accounting, S12 write-site enumeration, pin balance, collect ctx reset — all cross-confirmed sound). F1 [P3] = the PRE-EXISTING fs.c (void)-rollback orphan-inode class (EBUSY joins ENOMEM in the trigger set; leak-not-corruption, doubly-wedged-engine-only) -> tracked #369; F2 [P3] = the injection hook's relaxed load per concurrent emission -> closed-with-justification (deliberate, the eng_test_oom_countdown sibling, inert on serial walks); F3 [P3] = section 3.5's 2 MiB queued figure overstated the enforced 1 MiB floor -> FIXED in this doc. Report: .audit_r176_findings.md (parked). Closed list: memory audit_r176_closed_list.md. |
