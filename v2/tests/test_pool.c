...
STM_TEST(pool_fs_roundtrip_populates_roster) {
    ...
    /* P6-clone bumped STM_UB_VERSION 9 → 10 for the dataset-tree
     * on-disk value layout extension (origin_snap_id field, 8 bytes,
     * appended before name[]). Prior bumps: P6-persist (8 → 9) for
     * ub_main_root_gen + ub_snap_root_gen; P5-durable-cursors (7 → 8)
     * for ub_scrub_state[64]; P5-3c + R15 F6 (6 → 7) for the
     * roots-object leaf value layout. The constant symbol is what
     * we assert on; the literal 10 is restated here so a future
     * version bump that forgets to update this test fails loudly. */
    STM_ASSERT_EQ(stm_load_le32(ub.ub_version), STM_UB_VERSION);
    STM_ASSERT_EQ(STM_UB_VERSION, 10u);
