...
impl App {
    ...
    fn try_open_volume(&mut self, path: &str, _pass: Option<&str>, target: Focus) {
        // v2: `path` is treated as a Unix socket path served by an
        // already-running stratumd. Unlike v1 we don't spawn the
        // daemon ourselves — the user starts stratumd separately
        // (systemd / launchd / shell) and the TUI just dials in.
        // This is consistent with the v2 architecture: one daemon,
        // many client interfaces (TUI / CLI / FUSE).
        //
        // If `path` looks like a legacy `*.stm` volume image (file
        // doesn't end in `.sock` and isn't already a socket), we
        // surface a hint pointing the user at the v2 model.
        if !path.ends_with(".sock") {
            if let Ok(meta) = std::fs::metadata(path) {
                use std::os::unix::fs::FileTypeExt;
                if !meta.file_type().is_socket() {
                    self.status = format!(
                        "{path} is not a Unix socket — v2 expects a stratumd socket; \
                         start stratumd first then open the socket here."
                    );
                    return;
                }
            }
        ...
        match panel.connect_9p_unix(path) {
            Ok(()) => {
                panel.label = format!("[{}]", short_path(path));
                self.config.add_volume(path);
                self.status = format!("Connected to {path}");
            }
            Err(e) => {
    ...
    fn stop_server(&mut self) {
        // v2 doesn't manage daemon lifetime — the TUI only dials a
        // socket. stop_server is preserved as a no-op so callers
        // (close_panel, quit) compile unchanged. The server_proc /
        // server_sock fields stay None for the TUI's lifetime.
        let _ = &self.server_proc;
        let _ = &self.server_sock;
    }
    ...
    fn run_mkvol(&mut self, host_path: &str, _size_bytes: u64,
                 _passphrase: Option<&str>, _comp: CompAlgo, _target: Focus) {
        // v2: there's no `stratum mkfs` CLI yet (`stm_fs_format` is a
        // C API). Volume creation from the TUI lands in v1.1+ once a
        // mkfs CLI ships. For now surface a clear error.
        self.status = format!(
            "MkVolume {host_path}: not yet wired in v2 — stratum mkfs CLI \
             is forward-noted (v1.1)"
        );
    }
