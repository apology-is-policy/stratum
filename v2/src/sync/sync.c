...
struct stm_sync {
    ...
    /* TLY-A1 pool_serial — 16 bytes binding anchor. Written into
     * every uberblock by build_ub_for_target. At create: caller-
     * supplied (or CSPRNG-generated by stm_fs_format if caller passes
     * all-zero). At open: read from the authoritative uberblock during
                              ...
                              const stm_alloc_stats *astats)
{
    ...
    /* TLY-A1: pool_serial. Plaintext, in the un-encrypted superblock
     * header (must be readable before any decryption). All-zero =
     * "unbound" (back-compat with pre-Thylacine pools). */
    memcpy(out->ub_pool_serial, s->pool_serial, 16);
                            ...
                            stm_sync **out_sync)
{
    ...
    /* TLY-A1: pool_serial defaults to all-zero ("unbound" pool per
     * STRATUM-API-V1.md §3.2). Callers that want a bound pool call
     * stm_sync_set_pool_serial(s, csprng_16_bytes) BEFORE the first
     * stm_sync_commit so the value lands in the initial uberblock. */
     ...
     * (P5-3c) add more entries. */
    {
        ...
        /* 9.7-impl-1c-iii: dirent index. Same wiring shape as inode_idx
         * at 1c-ii — the module borrows the dataset index and resolves
         * each dataset's per-dataset btree_engine on demand. The
         * pool-global dirent tree under ub_dirent_root is RETIRED;
         * stamped zero at v30. */
        s->dirent_idx = stm_dirent_index_create();
        if (!s->dirent_idx) { stm_sync_close(s); return STM_ENOMEM; }
        rc = stm_dirent_index_attach_dataset_index(s->dirent_idx,
                                                    s->dataset_idx);
        if (rc != STM_OK) { stm_sync_close(s); return rc; }
                          ...
                          stm_sync **out_sync)
{
    ...
    /* TLY-A1: cache pool_serial from the canonical UB. Subsequent
     * commits will write the same value back via build_ub_for_target
     * (the field is write-once at format; never mutated after). */
    memcpy(s2->pool_serial, ub.ub_pool_serial, 16);
...
{
    if (!s || !serial) return;
    /* Caller's contract (sync.h docstring): must be called AFTER
     * stm_sync_create + BEFORE the first stm_sync_commit so the
     * value lands in the initial uberblock. Take the sync->lock so
     * a racing reader (e.g., the /ctl/ accessor) sees a consistent
     * snapshot — the field is 16 bytes so the memcpy isn't itself
     * atomic without the lock. */
    pthread_mutex_lock(&s->lock);
    memcpy(s->pool_serial, serial, 16);
...
{
    if (!s || !out) return;
    /* pool_serial is write-once at format; once cached at create OR
     * decode at open it never mutates. The sync->lock is taken
     * regardless because set_pool_serial is the legal pre-first-
     * commit setter and a /ctl/ read could in principle race a
     * format-time setter call before the first commit returns. */
    stm_sync *ms = (stm_sync *)s;
    pthread_mutex_lock(&ms->lock);
    memcpy(out, s->pool_serial, 16);
    pthread_mutex_unlock(&ms->lock);
}
