...
2. **Stage 2** (landed `b7a892f`) — delta log in isolation: per-sync allocator deltas appended to a backward-linked chain of 4 KiB log chunks; commit writes any pending entries as new chunks; fold-into applies entries to the tree. Unit-tested in `tests/test_space_log.c`. Not wired into the running fs either.
3. **Stage 3** (landed) — persistent-log infrastructure wired into the fs. SB gains `ss_space_log_head`, `ss_space_pool_start`, `ss_space_pool_blocks` fields. mkfs reserves a pool sized `min(2048, total_blocks/4)` blocks (min 32) starting just past the two SB slots, and seeds the log with an initial ALLOC for the fresh-tree root. Every `stm_alloc_extent` / `_free` / `_ref` now appends to the log (`_mark` does not — it's mount-walk reconstruction). `stm_fs_sync` commits the log between Phase 2 (tree flush) and Phase 3 (final SB write). `stm_fs_open` reads the SB, opens the log chain, attaches it to the allocator, and folds entries into a throwaway in-memory space tree to diagnose divergence from the walk-derived refcount array.
   - **Validation is WARN-only in Stage 3.** Known limitation: snapshot rollback leaves the log referencing ranges no longer reachable via the post-rollback tree. The walk rebuilds cleanly, the refcount array is the allocator's source of truth, but the log accumulates stale entries. Stage 3.5 will rebuild the log on rollback.
   - The reserved pool is never touched by the main allocator (via `stm_alloc_set_pool` which bumps pool-block refcounts to 1 and advances the allocator's wrap-hint past the pool). Log chunks are bump-allocated from the pool via `fs_space_log_alloc`, never through `stm_alloc_extent` (breaks the recursion).
4. **Stage 3.5** (next) — rebuild the log on rollback: truncate the on-disk chain, walk the post-rollback tree, emit fresh ALLOC entries for every live range. Re-enables strict validation (mismatches wedge the fs).
5. **Stage 4** — drop the full walk; space tree + log becomes source of truth. The O(`total_blocks`) refcount array is still maintained in memory during a session for fast allocation, but it's initialized from the persistent structure, not rebuilt from a walk.
6. **Stage 5** — on-demand space-tree queries replace the in-memory array entirely. RAM becomes O(distinct ranges) at runtime too. Focused audit covers every sync crash boundary.
