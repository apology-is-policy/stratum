...
- **Volume**: a pool's top-level namespace (conventional term for the root dataset).
- **Dataset** (aka subvolume): a nested namespace with independent snapshots, properties, and optionally keys. Per-dataset properties inherit from parents (§8.4).
- **Snapshot**: a frozen-in-time reference to a dataset's tree root, created O(1) via birth-txg tracking (§8.5).
- **Clone**: a writable dataset sharing initial state with a snapshot (§8.6).
- **Extent**: a contiguous byte range of file data — either **hot** (paddr-addressed at a specific device location, §6, §11.6) or **cold** (content-hash-addressed in the CAS tier, §6.9).
- **Chunk**: the content-defined unit in the cold tier. Boundaries determined by FastCDC (§6.9.2).
- **Transaction group (txg)**: a batch of writes committed atomically. Advances `write_gen` (§3.7, §5.6, §7.4).
- **Uberblock**: per-device superblock holding pool state for one txg. 4 KiB; 63-slot ring per label; 4 labels per device (§5.3).
- **Stripe**: the redundancy unit for RS/LRC profiles — a row of k+p (or k+l+g) blocks across devices (§4.6).

### 2.3 Relationships

The central objects and their references:

```
                     ┌─────────────────────┐
                     │     Uberblock       │  (per-device; ring of 63;
                     │  ub_main_root ──────┼─→ dataset index
                     │  ub_alloc_root ─────┼─→ allocator roots object
                     │  ub_snap_root ──────┼─→ snapshot index
                     │  ub_cas_index_root ─┼─→ CAS index
                     │  ub_merkle_root ────┼─→ (hash anchor for everything)
                     │  ub_key_schema ─────┼─→ wrapped dataset keys
                     │  ub_roster ─────────┼─→ device roster
                     └──────────┬──────────┘
                                │
         ┌──────────────────────┼──────────────────────┐
         ▼                      ▼                      ▼
  ┌─────────────┐      ┌───────────────┐      ┌──────────────┐
  │   Dataset   │      │   Snapshot    │      │     CAS      │
  │   Index     │      │    Index      │      │    Index     │
  └──────┬──────┘      └───────┬───────┘      └──────┬───────┘
         │                     │                     │
         │                     ▼                     │
  ...
  └──────┬──────┘                              └──────────────┘
         ...
         ▼                                            │
  ┌─────────────┐                                    │
  │   Inodes,   │    for cold extents, references────┘
  │   Dirents,  │
  │   Xattrs,   │
  │   Extents   │
  └─────────────┘
...
### 2.4 Invariants each abstraction maintains

| Abstraction | Invariant | Enforcement |
|---|---|---|
| Pool | `disk ss_gen > fs->gen` at mount and post-mount | §3.7, §5.6, TLA+ spec |
| Dataset | `di_parent_id` refers to existing dataset or 0 (root) | Insertion validation + scrub |
| Snapshot | `se_created_txg > 0` and ≤ current txg | §8.5.3 |
| Clone | `origin_snapshot_id` refers to existing snapshot with `has_clones` flag set | Insertion validation |
| Extent (hot) | `paddr + length ≤ device_size`; refcount ≥ 1 | Allocator (§6) |
| Extent (cold) | `hash` key exists in CAS index with refcount ≥ 1 | CAS GC (§6.9.3) |
| Uberblock | `csum` matches; `gen` monotonic; `roster_hash` consistent with device set | Mount quorum check (§5.5) |
| Transaction group | All ops in a txg committed atomically or not at all | Four-phase sync (§3.7.3, §5.6) |
...
*(Each subsystem documents its own on-disk format in its section. This section is the cross-cutting versioning policy: how formats evolve, how compat is maintained, what the stability promise means.)*

### 13.1 Goals and non-goals

**Goals**:

- v2.0 on-disk format is stable. Readers written at v2.0 keep working forever.
- Format evolution via feature flags without breaking backward readability (except for `incompat` flags).
- Self-describing: uberblock carries version + feature flags; any reader can tell whether it can handle the volume.
- Forward readability: an old reader encountering an unsupported feature refuses cleanly with a clear error message, not corruption or crash.

**Non-goals**:

- Backward compatibility with v1. Pre-v2.0 is throwaway (VISION §4.11). No migration tool; admins recreate from backups.
- Downgrade. Once upgraded to a higher version or with new flags, can't downgrade.
- Format flexibility for its own sake. Adding a feature flag is a commitment — we don't retire bits.

### 13.2 Decisions

From earlier docs:

- Pre-v2.0 throwaway (VISION §4.11, §5).
- ext4-style three-category feature flags (§5.9).

Decisions here:

- **Version field** `ub_version` in uberblock: monotonic integer. `1` at v2.0 release. Bumped only when the format's *structure* changes incompatibly (very rare).
- **Feature flags** are the primary evolution mechanism. Three 32-bit fields: `ub_flags_compat`, `ub_flags_ro_compat`, `ub_flags_incompat`.
- **Bits never reused**. A deprecated feature's bit stays allocated forever, documented in `docs/FEATURE-FLAGS.md`.
- **Little-endian on disk always**. All integer fields in structs.
- **Natural alignment** within packed structs. 8-byte fields at 8-byte boundaries, etc. Explicit padding where needed to preserve alignment.
- **10-year stability commitment**: a v2.0 volume mounts in v2.x code for at least 10 years post-release.

### 13.3 Feature flag taxonomy

Three categories, each 32 bits (room for 32 flags per category):

#### 13.3.1 `compat` flags

Old code can ignore this feature and still read and write the pool safely.

Used for:
- New optional telemetry (event log extensions).
- Cosmetic or observational changes.

Examples:
- `COMPAT_EXTENDED_EVENT_LOG` — richer event log format. Old code writes plain; new code writes structured. Pool stays readable to both.

#### 13.3.2 `ro_compat` flags

Old code can **read** but must not **write**. Writing with old code would break invariants.

Used for:
- Features that affect on-disk layout but only on writes.
- Features where old code's ignorance would cause slow corruption if it wrote.

Examples:
- `ROCOMPAT_LRC_REDUNDANCY` — pool uses LRC. Old code (RS-only) can read because data reconstruction still works, but must not write new blocks without LRC awareness.
- `ROCOMPAT_EXTENDED_PROPERTIES` — dataset has properties beyond v2.0's set. Old code reads with known properties; writes would clobber unknown ones.

#### 13.3.3 `incompat` flags

Old code cannot mount the pool at all.

Used for:
- Structural changes that old code would misinterpret.
- Security-critical changes where old code's behavior would be unsafe.

Examples:
- `INCOMPAT_EXT_PADDR` — paddr is 12 bytes instead of 8. Every extent record, every bptr changes size.
- `INCOMPAT_CAS_TIER` — cold tier is active. Old code doesn't understand content-addressed records.
- `INCOMPAT_MERKLE_V2` — Merkle tree structure changed (e.g., wider fan-out).

### 13.4 Version field vs feature flags

Most evolution uses feature flags. The `ub_version` field advances only for fundamental structure changes that feature flags can't express cleanly.

Rule of thumb:
- **New feature (adds a subsystem, a new record type, a new field)**: feature flag.
- **Incompatible breaking change to the SB layout, the tree structure, or the wire protocol at a low level**: version bump.

Expected cadence:
- `ub_version` bumps: rare, maybe once every 5-10 years.
- Feature flags: more frequent, as features land.

v2.x releases stay at `ub_version = 1`. A hypothetical v3.0 with a radically different layout would be `ub_version = 2`.

### 13.5 Registry

The canonical registry of allocated feature-flag bits lives in `docs/FEATURE-FLAGS.md`. Format:

```
| Bit | Category | Name                    | Introduced | Description |
|-----|----------|-------------------------|------------|-------------|
| 0   | compat   | EXTENDED_EVENT_LOG      | v2.1       | Structured event log |
| 0   | ro_compat| LRC_REDUNDANCY          | v2.0       | LRC erasure coding |
| 1   | ro_compat| EXTENDED_PROPERTIES     | v2.1       | >128 property bits   |
| 0   | incompat | CAS_TIER                | v2.0       | Cold tier active     |
| 1   | incompat | EXT_PADDR               | future     | 96-bit paddr         |
| ... | ...      | ...                     | ...        | ...                  |
```

Every PR that introduces a feature flag updates this registry. CI validates uniqueness.

### 13.6 Upgrade protocol

Admin-triggered: `stratum pool upgrade <pool>`:

1. Admin confirms (prompt).
2. Stratum enumerates feature flags the current binary supports but the pool doesn't have set.
3. For each flag, if it's `ro_compat` or `incompat`:
   - Admin confirms per-flag.
   - Flag is set in a new commit.
4. Pool's `ub_version` and feature flags advance.
5. Commit one txg with the new flags set.

Some `compat` flags auto-upgrade on next sync (no admin confirmation needed).

`incompat` upgrades are one-way; admin must understand that older readers will no longer mount.

### 13.7 Endian policy

All integer fields in on-disk structs are **little-endian**. Rationale:
- Matches x86-64 native byte order (most common deployment).
- ARM64 is bi-endian but defaults to little-endian (Linux, macOS).
- Big-endian machines (POWER, SPARC, s390x) byte-swap on read/write. Performance penalty is ~negligible modulo cache line crossing; byte-swap is a few cycles per field.

Mixed-endian pools (e.g., volume created on x86, mounted on POWER) work correctly; byte-swap happens at deserialization.

### 13.8 Alignment rules

Within packed structs (marked with `__attribute__((packed))`):
- Each field at its natural alignment: u64 at offset %8==0, u32 at offset %4==0.
- Explicit padding bytes where needed.
- Struct size padded to its own alignment.

Why pack at all: on-disk records are small; natural C alignment wastes bytes. Packed + explicit align ensures efficiency *and* compiler-portability.

### 13.9 The 10-year stability commitment

At v2.0 release:

- The uberblock format (field layout, offsets) is frozen. No reassignment of bytes.
- The Bε-tree node format (header layout, key/value encoding) is frozen.
- The extent record format is frozen.
- The allocator tree entry format is frozen.
- The CAS index entry format is frozen.
- The 9P wire protocol (including Stratum extensions) is frozen.

Changes happen only via feature flags. New fields are added in reserved regions. Existing fields are never repurposed.

**A pool created at v2.0 will mount in v2.x (x = 0..many) for at least 10 years**. After that, we commit to a migration path (not backward compat — explicit migration is acceptable at that scale).

### 13.10 Summary

1. v2.0 is the format baseline. Three-category feature flags (compat / ro_compat / incompat) for evolution.
2. Version field bumps only for fundamental structural changes (very rare).
3. Feature-flag bits never reused; documented in `docs/FEATURE-FLAGS.md`.
4. Upgrades are one-way admin operations. No downgrade.
5. Little-endian on disk; packed structs with natural-alignment padding.
6. 10-year stability commitment for v2.0-origin pools.
...
- Every subsystem exposes counters, events, and histograms via `/ctl/`.
- Structured event log for forensic review.
- Debug dumps (tree walks, extent maps, integrity reports) accessible without stopping the pool.
- Integration with external metrics systems (Prometheus, OpenTelemetry) via sidecar scrapers.
- Tracing with correlation IDs for deep debugging.

**Non-goals**:

- Built-in web UI / dashboard. External tools read `/ctl/`.
- Log shipping / aggregation. External tools' concern.
- Proprietary metrics protocols. Stick with open standards.

### 14.2 Decisions

From earlier docs:

- `/ctl/` synthetic filesystem (NOVEL #9, §10.5).

Decisions here:

- **Counter naming**: dotted hierarchy (`pool.<name>.ops.read.count`, etc.).
- **Event log** is per-pool, append-only, rotated at 100 MiB.
- **Tracing** is opt-in via `/ctl/tracing/enable`; samples at configurable rate.
- **Debug dumps** always available; some require admin privileges.
- **Metrics in Prometheus exposition format** by default; OpenTelemetry via adapter.

### 14.3 /ctl/ filesystem layout

```
/ctl/
├── state                               (read: pool state: OFFLINE / MOUNTING / ONLINE / DEGRADED / FAULTED / UNMOUNTING)
├── pools/
│   └── <pool_name>/
│       ├── status                       (health, redundancy state, capacity)
│       ├── devices/
│       │   └── <device_uuid>/
│       │       ├── status               (ONLINE / DEGRADED / FAULTED / REMOVED)
│       │       ├── class                (SSD / HDD / PMEM / ZNS)
│       │       ├── role                 (DATA / LOG / CACHE / SPARE)
│       │       ├── smart                (SMART data)
│       │       ├── scrub                (write: start/stop scrub for this device)
│       │       └── stats                (per-device IO counters)
│       ├── datasets/
│       │   └── <dataset_path>/
│       │       ├── properties           (read: current properties; write: set)
│       │       ├── stats                (per-dataset op counters)
│       │       ├── snapshots/           (directory of snapshot entries)
│       │       │   └── <snap_name>
│       │       ├── create-snapshot      (write: name → create)
│       │       └── rollback             (write: snap_name → rollback)
│       ├── scrub                        (write: start/stop/pause; read: progress)
│       ├── rebalance                    (write: start; read: progress)
│       ├── bootstrap-usage              (per-device bootstrap pool usage)
│       ├── commit-latency               (histogram: p50 / p99 / p99.9)
...
│   ├── errors                            (error log)
│   └── corruption                        (integrity-failure log)
├── tracing/
│   ├── enable                            (write: on / off)
│   ├── sample-rate                       (write: float 0..1)
│   └── traces                            (read: captured traces)
├── debug/
│   ├── tree-walk                         (write: path → dumps tree)
│   ├── extent-map                        (write: inode → dumps extents)
│   ├── allocator-state                   (per-device)
│   └── integrity-verify                  (write: path → triggers verify)
└── version                               (read: binary version + format version)
```

All paths support `cat` / `echo` / `ls` / `stat` semantics via 9P. Read paths report state; write paths trigger actions. Directory paths list children.

### 14.4 Counter schema

Counters use a dotted hierarchy:

```
pool.<name>.ops.read.count               uint64 — total reads
pool.<name>.ops.read.bytes               uint64 — total bytes read
pool.<name>.ops.read.latency.p50         uint64 — microseconds
pool.<name>.ops.read.latency.p99         uint64
pool.<name>.ops.read.latency.p99.9       uint64
pool.<name>.ops.write.count              uint64
pool.<name>.ops.write.bytes              uint64
pool.<name>.ops.write.latency.*          histogram
pool.<name>.ops.sync.count               uint64
pool.<name>.ops.sync.latency.*           histogram
pool.<name>.commit.count                 uint64 — txgs committed
pool.<name>.commit.latency.*             histogram
pool.<name>.device.<uuid>.read.count     uint64
pool.<name>.device.<uuid>.read.bytes     uint64
pool.<name>.device.<uuid>.write.count    uint64
pool.<name>.device.<uuid>.write.bytes    uint64
pool.<name>.device.<uuid>.errors         uint64
pool.<name>.scrub.completed              uint64 — count of completed scrubs
pool.<name>.scrub.errors_found           uint64
pool.<name>.scrub.errors_repaired        uint64
pool.<name>.dataset.<path>.ops.*         per-dataset op counters
pool.<name>.mem.allocator_bytes          uint64 — in-RAM allocator state
pool.<name>.mem.cache_bytes              uint64 — W-TinyLFU cache usage
pool.<name>.mem.fixed_buffers_bytes      uint64 — io_uring fixed buffer pool
...
```

Histograms expose percentiles and bucket counts. Per-device / per-dataset counters have their `<uuid>` / `<path>` substituted.

Readable as individual files (`cat pool.tank.ops.read.count`) or as bulk dumps (`cat metrics/prometheus`).

### 14.5 Event log

Per-pool, append-only, structured. Rotation at 100 MiB with gzipped archives retained for 30 days.

Format: one event per line, JSON-encoded:

```json
{"ts":"2026-04-19T14:23:45.123456Z","event":"scrub_start","pool":"tank","priority":"low"}
{"ts":"2026-04-19T14:28:11.999999Z","event":"corruption_detected","pool":"tank","paddr":"0x1a2b3c4d","kind":"merkle_mismatch","action":"repair_attempted"}
{"ts":"2026-04-19T14:28:12.001234Z","event":"repair_success","pool":"tank","paddr":"0x1a2b3c4d","source":"mirror_peer_device"}
{"ts":"2026-04-19T14:30:00.555555Z","event":"device_added","pool":"tank","device":"nvme0n2","role":"DATA","class":"SSD"}
{"ts":"2026-04-19T14:35:00.777777Z","event":"key_unwrap","pool":"tank","dataset":"tank/home","requester_uid":1000,"agent_backend":"passphrase"}
```

Event types (non-exhaustive):
- `scrub_start`, `scrub_complete`, `scrub_pause`
- `repair_success`, `repair_failed`
- `corruption_detected`
- `device_added`, `device_removed`, `device_faulted`
- `mount`, `unmount`
- `commit_slow` (if commit exceeded latency budget)
- `snapshot_created`, `snapshot_deleted`, `rollback`
- `key_unwrap`, `key_rotate`
- `quorum_lost`, `quorum_restored`
- `degraded_state_entered`, `online_state_restored`

### 14.6 Tracing

Opt-in per-request distributed-tracing-style spans with correlation IDs.

```
$ echo "on" > /ctl/tracing/enable
$ echo "0.1" > /ctl/tracing/sample-rate    # 10% sampling
```

Each traced op gets a correlation ID; spans are emitted at every subsystem boundary:

```
trace[abc123]: op=Tread fid=42 offset=0 len=4096
  span[§10.4]: fuse_translate_read(42, 0, 4096)                  [25 µs]
    span[§3.3]: mvcc_reader_enter(epoch=5678)                    [1 µs]
    span[§3.4]: tree_lookup(dataset=tank/home, key=...)          [45 µs]
      span[§7.13.2]: verify_merkle_path(depth=4)                 [8 µs]
    span[§9.4]: io_uring_read(paddr=..., len=4096)               [120 µs]
    span[§7.5]: aead_decrypt(algo=aegis-256)                      [3 µs]
  span[§10.4]: fuse_reply(data_len=4096)                          [5 µs]
total: 207 µs
```

Traces readable via `cat /ctl/tracing/traces`; can pipe to OpenTelemetry exporter.

### 14.7 Debug dumps

Detailed state for troubleshooting:

- **Tree walk**: `echo "/tank/home" > /ctl/debug/tree-walk` dumps the file/dir's btree structure to the response buffer. Shows node addresses, child pointers, key distribution.
- **Extent map**: `echo "/tank/home/file.bin" > /ctl/debug/extent-map` dumps the file's extent list: offset, length, paddr, compression, csum.
- **Allocator state**: `cat /ctl/debug/allocator-state/<device_uuid>` dumps the device's allocated ranges + bootstrap pool usage.
- **Integrity verify**: `echo "/tank/home" > /ctl/debug/integrity-verify` triggers a full Merkle + per-extent verify of the path; results readable subsequently.

Debug ops are admin-gated (require admin UID via SO_PEERCRED).

### 14.8 Metrics integration

#### 14.8.1 Prometheus

`/ctl/pools/<name>/metrics/prometheus` returns Prometheus exposition format:

```
# HELP stratum_pool_ops_read_count Total reads.
# TYPE stratum_pool_ops_read_count counter
stratum_pool_ops_read_count{pool="tank"} 1234567

# HELP stratum_pool_ops_read_latency_seconds Read latency.
# TYPE stratum_pool_ops_read_latency_seconds histogram
stratum_pool_ops_read_latency_seconds_bucket{pool="tank",le="0.0001"} 100000
stratum_pool_ops_read_latency_seconds_bucket{pool="tank",le="0.001"} 1000000
...
```
...
scrape_configs:
  - job_name: stratum
    ...
    metrics_path: /ctl/pools/tank/metrics/prometheus
```

Wait — Prometheus scrapes HTTP, not 9P. A small sidecar (`stratum-metrics-exporter`) translates 9P `/ctl/` reads to HTTP responses.

#### 14.8.2 OpenTelemetry

Similar: `/ctl/pools/<name>/metrics/opentelemetry` returns OTLP-format metrics. Sidecar translates to HTTP / gRPC if needed.
...
Concerns that span multiple subsystems. Not subsystem-owned; live here as policy.

### 15.1 Feature-flag lifecycle

Feature flags have three phases:

1. **Introduction**: a PR adds a feature flag, implements the feature behind it, updates `docs/FEATURE-FLAGS.md`. Flag is `EXPERIMENTAL` — users opt in knowing it might change.
2. **Stabilization**: after one minor version (e.g., v2.1 → v2.2), the flag transitions to `STABLE`. Format is frozen; backward compat guaranteed.
3. **Deprecation**: a flag can be marked `DEPRECATED` (new pools don't set it; old pools still use it). `OBSOLETE` means "this flag's feature is retired"; old pools set with this bit get a mount warning but still work.

Bits are never reused. Obsolete bits stay in the registry marked as such.

### 15.2 Error model

A three-tier error taxonomy (from §9.11, generalized):

- **Recoverable**: caller can retry; underlying state unchanged. Examples: `EAGAIN`, `EINTR`, `ENOSPC` (user can free space).
- **Transient unexpected**: likely environmental; retry with backoff. Examples: transient I/O error.
- **Permanent failure**: caller cannot recover without admin intervention. Examples: corrupted metadata, unrecoverable crypto failure.

Error propagation:
- Every layer returns POSIX-compatible errnos.
- Additional context via 9P `Rerror` message or via structured error log (`/ctl/logs/errors`).
- Permanent failures log to `/ctl/logs/errors` AND `/ctl/events` AND raise an alert (admin notification via configured mechanism — email, webhook, etc.).

### 15.3 Resource limits

Stratum imposes and enforces resource limits:

- **Memory**: configurable caps per subsystem.
  - W-TinyLFU cache: 64 MiB default, admin-settable.
  - io_uring fixed-buffer pool: 128 MiB default.
  - Allocator in-RAM state: ~25 MiB per TiB of data (target §6.6).
  - 9P connection state: bounded per-connection (fids, namespace entries).
- **I/O concurrency**: io_uring submission queue depth (512 default). Concurrent commits: 1 (serialized coordinator).
- **CPU**: no hard cap (let the OS schedule), but scrub + rebalance are IO-throttled.
- **File descriptors**: bounded by `nofile` ulimit + 9P session fid table.

Limits exposed at `/ctl/limits/` — readable (current values) and writable (admin adjusts).

### 15.4 Cancellation and timeouts

Long-running operations (scrub, rebalance, tier migration, send / recv) support cancellation:

- All long ops check for cancellation periodically (every ~100 ms).
- Cancellation signal: `echo "cancel" > /ctl/.../scrub` (or equivalent).
- On cancel, op pauses cleanly at a checkpoint; can resume or be abandoned.

Timeouts:
- Client-side: 9P `Tread` / `Twrite` have no explicit timeout; clients implement their own.
- Server-side: operations acquiring long-held locks (EBR epochs, §3.6) time out at 30 seconds; beyond that, the thread is flagged as stalled and its epoch is force-released (with the thread's next op failing).

### 15.5 Backward compatibility

Policy per §13:

- Within v2.x, any binary reads and writes all v2.x pools (subject to feature flags).
- Across major versions (hypothetical v3.0): admins run a migration tool (not in-place).
- Old tooling can read v2.x pools that don't set any `ro_compat` or `incompat` flags it doesn't understand; old tooling refuses to mount pools with unknown `ro_compat` flags.

### 15.6 Thread / task model

Threads stratum creates at runtime:

| Role | Count | Purpose |
|---|---|---|
| 9P server loop | 1 | Accepts connections, dispatches to handlers |
| 9P client handlers | 1 per connection | Per-client request processing |
| Commit coordinator | 1 | Serializes commits (§3.7) |
| Phase 2 workers | N (config; default = cpu_count) | Parallel node consolidation during commit |
| io_uring completion | 1 per device ring pair | Polls CQE, dispatches callbacks |
| Scrub workers | 1 per device (when scrub active) | Walks metadata + extents |
| Rebalance workers | 1 per device (when rebalance active) | Moves data between devices |
| Background consolidator | 1 (optional, §3.4.3) | Consolidates delta chains in read-heavy workloads |
| Tier migration | 1 (when tiering active) | Moves extents hot↔cold |

Coordination:
- Writers + coordinator synchronize via the commit CAS (§3.7.2).
- Workers within a phase run independently; results joined at phase boundary.
- Background workers run at low priority (nice / cpuset + io-weight).

### 15.7 Backpressure

When workload exceeds capacity, Stratum applies backpressure rather than unbounded buffering:

- **Commit pressure**: if dirty delta bytes exceed 128 MiB threshold, triggers commit. If commits can't keep up (writes > disk throughput), writes block at `Twrite` until commit frees buffer space.
- **Fixed-buffer pool pressure**: if all 1024 buffers are in flight, new submissions queue in io_uring SQ (up to 512 entries per ring). Beyond that, new submissions block.
- **Quorum pressure**: if too few devices online for quorum, commits fail; writes reject with `-EROFS`.

Backpressure is preferable to unbounded buffering — the system remains responsive under overload rather than OOMing.

### 15.8 Startup and shutdown ordering

Daemon startup:
1. Parse config, load key agent connection config.
2. Open block devices (read labels, validate).
3. Mount pool(s) per §12.8.
4. Start 9P server on configured endpoints.
5. Signal readiness (systemd notify, etc.).

Daemon shutdown:
1. Stop accepting new connections.
2. Per §12.9 unmount protocol.
3. Flush all commits.
4. Close devices.
5. Exit.

On signal-induced shutdown (SIGTERM): same as clean shutdown but with a timeout (default 30 s); forced unmount beyond timeout.

On SIGKILL: no cleanup. On next start, crash recovery (§12.12) activates.
...
| §13 Format versioning | COMMITTED | 11 |
| §14 Observability | COMMITTED | 12 |
| §15 Cross-cutting | COMMITTED | 12 |
| §16 Appendices | STUB (grows continuously) | — |

**ARCHITECTURE.md is complete as of this commit.** §16 appendices (glossary, algorithm pseudocode, bibliography) grow organically as concrete implementation details are specified.
